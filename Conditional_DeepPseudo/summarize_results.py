...
This 'summarize_results.py' gives the final outputs on test data.
'''
...
from Conditional_DeepPseudo import Conditional_DeepPseudo_Model
...
args = init_arg()
odir = 'Synthetic'
logger = utils.init_logger(odir, 'log_conditional_deeppseudo_sum.txt')
...
for out_itr in range(OUT_ITERATION):
    ...
    tr_data, tr_time, tr_label, y_train, va_data, va_time, va_label, te_data, te_time, te_label, num_Category, num_Event, num_evalTime, x_dim =import_data(0, evalTime, categorical_columns=None, continuous_columns=continuous_columns)
    ...
    model = Conditional_DeepPseudo_Model(sess, "Conditional_DeepPseudo", input_dims, network_settings)
    ...
    y_pred_con = model.predict(te_data)
    y_pred_con= y_pred_con.ravel()
    y_pred_con1=y_pred_con[:int(len(y_pred_con)/2)]
    y_pred_con2=y_pred_con[int(len(y_pred_con)/2):]
    pred=np.zeros(shape=(int(len(va_data)/4),num_Event, len(evalTime)))
    ...
    pred_br=np.minimum(1, np.maximum(0, pred))
