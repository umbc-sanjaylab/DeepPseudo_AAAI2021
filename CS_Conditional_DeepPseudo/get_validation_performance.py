...
This 'get_validation_performance.py' trains the Cause-specific Conditional DeepPseudo model and give the validation C-index performance for random search.
"""
import numpy as np
import pandas as pd
...
def get_valid_performance(in_parser, out_itr, evalTime=None, MAX_VALUE = -99, OUT_ITERATION=5):
    ...
    tr_data, tr_time, tr_label, y_train1, y_train2, va_data, va_time, va_label, te_data, te_time, te_label, num_Category, num_Event, num_evalTime, x_dim =import_data(out_itr, evalTime, categorical_columns=None, continuous_columns=continuous_columns)
    ...
    for itr in range(iteration):
        if stop_flag > 10: #for faster early stopping
            ...
            if (itr+1)%1000 == 0:
                ...
                y_pred_con1 = model.predict1(va_data)
                y_pred_con1 = y_pred_con1.ravel()
                y_pred_con2 = model.predict2(va_data)
