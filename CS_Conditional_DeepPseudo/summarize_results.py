...
This 'summarize_results.py' gives the final outputs of Cause-specific Conditional DeepPseudo model on test data.
'''
...
from CS_Conditional_DeepPseudo import CS_Conditional_DeepPseudo_Model
...
for out_itr in range(OUT_ITERATION):
    ...
    tr_data, tr_time, tr_label, y_train1, y_train2, va_data, va_time, va_label, te_data, te_time, te_label, num_Category, num_Event, num_evalTime, x_dim =import_data(0, evalTime, categorical_columns=None, continuous_columns=continuous_columns)
    ...
    network_settings            = { 'num_units_shared'   : num_units_shared,
                                    'num_layers_shared'  : num_layers_shared,
                                    'num_units_CS'       : num_units_CS,
    ...
    model = CS_Conditional_DeepPseudo_Model(sess, "CS_Conditional_DeepPseudo", input_dims, network_settings)
    ...
    y_pred_con1 = model.predict1(te_data)
    y_pred_con1 = y_pred_con1.ravel()
    y_pred_con2 = model.predict2(te_data)
    y_pred_con2 = y_pred_con2.ravel()
    pred=np.zeros(shape=(int(len(va_data)/2),num_Event, len(evalTime)))
    pred[:,0,0]=y_pred_con1[:int(len(y_pred_con1)/2)]
    pred[:,0,1]=y_pred_con1[int(len(y_pred_con1)/2):]
    pred[:,1,0]=y_pred_con2[:int(len(y_pred_con2)/2)]
    pred[:,1,1]=y_pred_con2[int(len(y_pred_con1)/2):] 
    ...
    print('SharedNet Parameters: ' + 'num_units_shared = '+str(num_units_shared) + ' num_layers_shared = '+str(num_layers_shared)+' CSNet Parameters: ' + ' num_units_CS = '+str(num_units_CS) + ' num_layers_CS = '+str(num_layers_CS) + ' Non-Linearity: ' + str(activation_fn)+ ' Learning Rate: ' + str(lr_train))
