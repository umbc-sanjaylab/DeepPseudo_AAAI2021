""" The code is inspired by the code for DeepHit model. The github link of the code for DeepHit is https://github.com/chl8856/DeepHit. Reference: C. Lee, W. R. Zame, J. Yoon, M. van der Schaar, "DeepHit: A Deep Learning Approach to Survival Analysis with Competing Risks," AAAI Conference on Artificial Intelligence (AAAI), 2018.
   ...
   ## Cause-specific Conditional DeepPseudo model's architecture:
...
class CS_Conditional_DeepPseudo_Model:
    ...
    def _build_net(self):
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            self.y1         = tf.placeholder(tf.float32, shape=[None, 1], name='pseudo1') #Pseudo values for CIF for cause 1
            self.y2         = tf.placeholder(tf.float32, shape=[None, 1], name='pseudo2') #pseudo values for CIF for cause 2
            ...
            self.out1= FC_Net(cs_out1, 1, activation_fn=tf.nn.selu, 
            ...
            self.out2= FC_Net(cs_out2, 1, activation_fn=tf.nn.selu, 
    ...
    ## Prediction Function for cause 1
    def predict1(self, x_test, keep_prob=1.0):
        return self.sess.run(self.out1, feed_dict={self.x: x_test, self.mb_size: np.shape(x_test)[0], self.keep_prob: keep_prob})
    ## Prediction Function for cause 2
    def predict2(self, x_test, keep_prob=1.0):
        return self.sess.run(self.out2, feed_dict={self.x: x_test, self.mb_size: np.shape(x_test)[0], self.keep_prob: keep_prob})
